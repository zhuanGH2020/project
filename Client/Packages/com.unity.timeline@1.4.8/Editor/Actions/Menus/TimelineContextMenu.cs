...
namespace UnityEditor.Timeline
{
    static class SequencerContextMenu
    {
        ...
        private static bool CheckTrackTypeUnavailable(UnityEngine.Object asset, string identifier)
        {
            try
            {
                System.Type type = System.Type.GetType("SeasonVersionEditChecker, Assembly-CSharp-Editor");
                MethodInfo methodInfo = type.GetMethod("CheckClassProxyUnavailable", BindingFlags.Static | BindingFlags.Public);
                return (bool)methodInfo.Invoke(null, new object[] { asset, identifier });
            }
            ...
            foreach (var trackType in allTypes)
            {
                ...
                menuItems.Add(
                    new MenuActionItem()
                    {
                        ...
                        isActiveInMode = TimelineEditor.inspectedDirector == null ? true : CheckTrackTypeUnavailable(TimelineEditor.inspectedDirector.gameObject, trackItemType.FullName) == false,
        ...
        static string GenerateMenuEntryName(string commandNameTemplate, Type assetType, Type objectReferenceType)
        {
            var assetTypeDisplayName = TypeUtility.GetDisplayName(assetType);
            var objectReferenceTypeDisplayName = TypeUtility.GetDisplayName(objectReferenceType);
            if (assetTypeDisplayName.StartsWith("Skill"))
                return string.Format("Skill Fx/Add {0}", assetTypeDisplayName);
            else
                return string.Format(commandNameTemplate, assetTypeDisplayName, objectReferenceTypeDisplayName);
        }

        static int GenerateMenuEntryPriority(Type assetType, int builtInPriority, int customPriority)
        {
            var assetTypeDisplayName = TypeUtility.GetDisplayName(assetType);
            if (assetTypeDisplayName.StartsWith("Skill"))
                return MenuPriority.BattleSkillEditor.defaultValue;
            else
                return TypeUtility.IsBuiltIn(assetType) ? builtInPriority : customPriority;
        }
            ...
            foreach (var assetType in assetTypes)
            {
                ...
                foreach (var objectReference in TypeUtility.ObjectReferencesForType(assetType))
                {
                    ...
                    menuItems.Add(
                        ...
                        {
                            category = category,
                            entryName = GenerateMenuEntryName(commandNameTemplate, assetType, objectReference.type),
                            isActiveInMode = true,
                            priority = GenerateMenuEntryPriority(assetType, builtInPriority, customPriority),
                            state = enabled ? ActionValidity.Valid : ActionValidity.Invalid,
                            callback = menuCallback
                        }
                    );
