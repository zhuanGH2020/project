...
namespace UnityEditor.Timeline
{
    static class AnimationTrackExtensions
    {
        ...
        static string GenerateBattleSkillEditorClipPath(string timelineAssetPath)
        {
            var index = 0;
        ...
        public static void BattleSkillEditorConvertToClipMode(this AnimationTrack track)
        {
            if (!track.CanConvertToClipMode())
                return;
            ...
            if (!track.infiniteClip.empty)
            {
                ...
                var assetPath = GenerateBattleSkillEditorClipPath(UnityEditor.AssetDatabase.GetAssetPath(track));
                UnityEditor.AssetDatabase.CreateAsset(GameObject.Instantiate(animClip), assetPath);

                AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport | ImportAssetOptions.ForceUpdate);

                var clip = track.CreateClip(UnityEditor.AssetDatabase.LoadAssetAtPath<AnimationClip>(assetPath));
            ...
            Object.DestroyImmediate(track.infiniteClip, true); // 光置Null不够，需要销毁
            track.infiniteClip = null;
