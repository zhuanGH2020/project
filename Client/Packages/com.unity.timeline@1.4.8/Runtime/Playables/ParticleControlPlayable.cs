...
#if UNITY_EDITOR
            ParticleTrackUtil.AddParticle(particleSystem);
#endif
        }
...
#if UNITY_EDITOR
    public static class ParticleTrackUtil
    {
        private static float lastTime;

        public static int currentParticleCount
        {
            get
            {
                var count = 0;
                foreach (var ps in timelineParticleSystems)
                {
                    if (ps != null)
        ...
        private static HashSet<ParticleSystem> timelineParticleSystems = new ();

        public static void AddParticle(ParticleSystem ps)
        {
            var curTime = Time.time;
            if (curTime != lastTime)
            {
                lastTime = curTime;
                RemoveNullParticleSystem();
            }
            timelineParticleSystems.Add(ps);
        }
        
        public static void RemoveNullParticleSystem()
        {
            timelineParticleSystems.RemoveWhere(ps => ps == null);
        }
        
        public static void Clear()
        {
            timelineParticleSystems.Clear();
        }
    }
#endif
}
