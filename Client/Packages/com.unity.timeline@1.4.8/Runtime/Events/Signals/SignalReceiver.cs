...
namespace UnityEngine.Timeline
{
    ...
    public class SignalReceiver : MonoBehaviour, INotificationReceiver
    {
        ...
        [SerializeField]
        EventKeyValue<double> m_FrameEvents = new EventKeyValue<double>();
        ...
        public void OnNotify(Playable origin, INotification notification, object context)
        {
            ...
            {
                if (m_Events.TryGetValue(signal.asset, out var evt))
                {
                    evt?.Invoke();
                }

                if (m_FrameEvents.TryGetValue(signal.asset, out var frameEvt))
                {
                    frameEvt?.Invoke(signal.time);
                }
            }
        }
        ...
        public void AddFrameReaction(SignalAsset asset, UnityEvent<double> reaction)
        {
            if (asset == null)
                throw new ArgumentNullException("asset");

            if (m_FrameEvents.signals.Contains(asset))
                throw new ArgumentException("SignalAsset already used.");
            
            m_FrameEvents.Append(asset, reaction);
        }
        ...
        public void RemoveFrameEvent(SignalAsset asset)
        {
            if (!m_FrameEvents.signals.Contains(asset))
            {
                throw new ArgumentException("The SignalAsset is not registered with this receiver.");
        ...
        {
            if (m_Events.TryGetValue(key, out var ret))
            {
                return ret;
            }
        ...
        public UnityEvent<double> GetFrameReaction(SignalAsset key)
        {
            if (m_FrameEvents.TryGetValue(key, out var ret))
            {
                return ret;
            }
        ...
        [Serializable]
        class EventKeyValue<T>
        {
            [SerializeField]
            List<SignalAsset> m_Signals = new List<SignalAsset>();

            [SerializeField, CustomSignalEventDrawer]
            List<UnityEvent<T>> m_Events = new List<UnityEvent<T>>();

            public bool TryGetValue(SignalAsset key, out UnityEvent<T> value)
            {
                var index = m_Signals.IndexOf(key);
                if (index != -1)
                {
            ...
            public void Append(SignalAsset key, UnityEvent<T> value)
            {
                m_Signals.Add(key);
                m_Events.Add(value);
            }
            ...
            public List<UnityEvent<T>> events => m_Events;
        }
    }
}
