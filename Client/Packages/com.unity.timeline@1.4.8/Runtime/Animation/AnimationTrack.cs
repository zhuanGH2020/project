...
namespace UnityEngine.Timeline
{
    ...
    public partial class AnimationTrack : TrackAsset, ILayerable
    {
        ...
        public class AnimatorTriggerBehaviour : PlayableBehaviour
        {
            ...
            [NonSerialized]
            public PlayableDirector RuntimeDirector;

            private bool _animatorTriggered;
            ...
            public override void PrepareFrame(Playable playable, FrameData info)
            {
                ...
                {
                    //maybe the clip is null
                    return;
                }
            
                input.SetTime(playable.GetTime());
                input.SetDuration(playable.GetDuration());
                
                var currentTime = RuntimeDirector.time;

                // in playing
                if (currentTime > OwningClip.start && currentTime < OwningClip.end)
                {
                    if (!OutputAnimator.gameObject.activeInHierarchy && _animatorTriggered)
                    {
                        _animatorTriggered = false;
                    }

                    if (OutputAnimator != null && OwningClip != null && !_animatorTriggered && OutputAnimator.gameObject.activeInHierarchy)
                    {
                        if (OutputAnimator.runtimeAnimatorController != null)
                        {
                            OutputAnimator.SetTrigger(Trigger);
                        }
        ...
        Playable CompileTrackPlayable(PlayableGraph graph, AnimationTrack track, GameObject go, IntervalTree<RuntimeElement> tree, AppliedOffsetMode mode)
        {
            ...
            var myDirector = go.GetComponent<PlayableDirector>();
            var binding = myDirector.GetGenericBinding(track);
            
            Animator myAnimator = null;
            if (binding is GameObject bindingObj)
            {
                myAnimator = bindingObj.GetComponent<Animator>();
            }
            else if (binding is Animator animator)
            {
                myAnimator = animator;
            }
            ...
            for (int i = 0; i < track.clips.Length; i++)
            {
                ...
                if (animationAsset != null)
                {
                    ...
                    animationAsset.SetTrackInfos(track, myDirector, c, myAnimator);
                }
                ...
                if (source.IsValid())
                {
                    if (myAnimator != null && animationAsset != null && !string.IsNullOrEmpty(animationAsset.m_Trigger))
                    {
                        var animationPlayBehaviour = ScriptPlayable<AnimatorTriggerBehaviour>.Create(graph, 1);
                        var animBehaviour = animationPlayBehaviour.GetBehaviour();
                        animBehaviour.Trigger = animationAsset.m_Trigger;
                        animBehaviour.OwningClip = c;
                        animBehaviour.OutputAnimator = myAnimator;
                        animBehaviour.RuntimeDirector = myDirector;
                        graph.Connect(source, 0, animationPlayBehaviour, 0);
                        graph.Connect(animationPlayBehaviour, 0, mixer, i);
                    }
                    else
                    {
