...
namespace TMPro.EditorUtilities
{
    ...
    {
        static ShaderFeature s_OutlineFeature, s_UnderlayFeature, s_BevelFeature, s_GlowFeature, s_MaskFeature, s_ClipAppearFeature;

        static bool s_Face = true, s_Outline = true, s_PlayerOnScreen, s_Outline2, s_Underlay, s_Lighting, s_Glow, s_Bevel, s_Light, s_Bump, s_Env, s_ClipAppear;
        ...
        static TMP_SDFShaderGUI()
        {
            ...
            s_ClipAppearFeature = new ShaderFeature()
            {
                undoLabel = "Glow",
                keywords = new[] { "UEP_VARIANT_1" }
            };
        ...
        {
            if (m_Material.HasProperty(ShaderUtilities.ID_ZTestMode))
            {
                MaterialProperty property = FindProperty("_AlwaysTop", m_Properties);
                MaterialProperty realValue = FindProperty("_ZMode", m_Properties);
                MaterialProperty curved = FindProperty("_CurveOn", m_Properties);
                EditorGUI.BeginChangeCheck();
                EditorGUI.showMixedValue = property.hasMixedValue;
                m_Editor.BeginAnimatedCheck(Rect.zero, property);

                bool value = EditorGUILayout.Toggle(new GUIContent("显示在最前"), property.floatValue == 1f);
                bool cValue = EditorGUILayout.Toggle(new GUIContent("受Curve影响"), curved.floatValue == 1f);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    realValue.floatValue = value ? 8f : 4f;

                    curved.floatValue = cValue ? 1f : 0f;
                }
                //DoToggle("_FaceShininess", "ZTestMode");
            }
            ...
            if (m_Material.HasProperty(ShaderUtilities.ID_NamebarBGTex))
            {
                s_PlayerOnScreen = BeginPanel("PlayerOnScreen", s_PlayerOnScreen);
                if (s_PlayerOnScreen)
                { 
                    DoPlayerOnScreenPanel();
                }
                EndPanel();
            }
        ...
        void DoPlayerOnScreenPanel()
        {
            EditorGUI.indentLevel += 1;

            if (m_Material.HasProperty(ShaderUtilities.ID_NamebarBGTex))
            {
                DoTexture2D("_NameBarBgTex", "Texture");
            }
        ...
        void DoDebugPanel()
        {
            ...
            if(m_Material.HasProperty(ShaderUtilities.ID_MaxColor))
                DoSlider("_MaxColor", "MaxColor");
            ...
            if (m_Material.HasProperty(ShaderUtilities.ID_ExtraScale))
            {
                EditorGUILayout.Space();
                DoFloat("_ExtraScale", "Extra Scale");
            }
            ...
            // Add by uitc
            if (m_Material.HasProperty("_UniformScaleUI") && m_Material.HasProperty("_BaseScaleSize"))
            {
                EditorGUILayout.Space();
                DoToggle("_UniformScaleUI", "Uniform Scale UI");
                DoFloat("_BaseScaleSize", "Base Scale Size");
            }
        ...
        {
            EditorGUI.indentLevel += 1;
            //DoFloat("_LineTime", "_LineTime(单行时间)");
            //DoFloat("_LineGap", "_LineGap(错位时间)");
            //DoSlider("_ClipOffsetY", "_ClipOffsetY(遮挡进度)");
            EditorGUI.indentLevel -= 1;
        }
