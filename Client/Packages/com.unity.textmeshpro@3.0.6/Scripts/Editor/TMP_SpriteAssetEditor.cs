...
namespace TMPro.EditorUtilities
{
    ...
    public class TMP_SpriteAssetEditor : Editor
    {
        ...
        ReorderableList m_alternativeMaterialsList;
        ...
        public void OnEnable()
        {
            ...
            m_alternativeMaterialsList = new ReorderableList(serializedObject, serializedObject.FindProperty("m_AlternativeMaterials"), true, true, true, true);

            m_alternativeMaterialsList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                var element = m_alternativeMaterialsList.serializedProperty.GetArrayElementAtIndex(index);
                rect.y += 2;
                EditorGUI.PropertyField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), element, GUIContent.none);
            };

            m_alternativeMaterialsList.drawHeaderCallback = rect =>
            {
                EditorGUI.LabelField(rect, new GUIContent("替换材质列表", "可供选择的材质列表."));
            };
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (UI_PanelState.spriteAtlasInfoPanel)
            {
                ...
                m_alternativeMaterialsList.DoLayoutList();
                EditorGUILayout.Space();
            }
            #endregion
