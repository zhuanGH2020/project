...
namespace TMPro
{
    ...
    public class TMP_FontAsset : TMP_Asset
    {
        ...
        /// <summary>
        /// 多人同屏时，所有字形写入同一Asset下的AtlasTexture. Add by UITC.
        /// </summary>
        public bool packAtlasTexture
        {
            get { return m_packAtlasTexture; }
            set { m_packAtlasTexture = value; }
        }
        [SerializeField]
        private bool m_packAtlasTexture;
        ...
        /// <summary>
        /// 用于Player On Screen字填充失败（填满）.
        /// </summary>
        public virtual event Action<TMP_FontAsset> OnFontAssetTextureAddFailed = delegate { };
        ...
        private static string m_fallbackFontNameStart = "fallback";
        ...
        public bool HasCharacter(char character, bool searchFallbacks = false, bool tryAddCharacter = false)
        {
            ...
            if (tryAddCharacter && m_AtlasPopulationMode == AtlasPopulationMode.Dynamic)
            {
                ...
                if (TryAddCharacterInternal(character, false, packAtlasTexture ? this : null, out returnedCharacter))
                    return true;
            }
        ...
        */
        ...
        /// <param name="character">The character data if successfully added to the font asset. Null otherwise.</param>
        /// <returns>Returns true if the character has been added. False otherwise.</returns>
        internal bool TryAddCharacterInternal(uint unicode, bool useOSFontFallback, TMP_FontAsset targetFontAsset, out TMP_Character character)
        {
            ...
            if (!useOSFontFallback && m_MissingUnicodesFromFontFile.Contains(unicode))
            {
            ...
            int targetOSFallbackFontIndex = -1;
            Font targetFontFile = null;

            // Load font face.
            if (FontEngine.LoadFontFace(sourceFontFile, m_FaceInfo.pointSize) != FontEngineError.Success && !useOSFontFallback)
            {
                k_TryAddCharacterMarker.End();
                return false;
            }
            ...
            if (glyphIndex == 0)
            {
                ...
                if (glyphIndex == 0)
                {
                    if (useOSFontFallback)
                    {
                        for (int i = 0; i < TMP_Settings.fallbackFontPaths.Count; i++)
                        {
                            if (targetFontAsset != null && FontEngine.LoadFontFace(TMP_Settings.fallbackFontPaths[i], targetFontAsset.faceInfo.pointSize, TMP_Settings.fallbackFontFaceIndex[i]) != FontEngineError.Success)
                            {
                                continue;
                            }

                            else if (FontEngine.LoadFontFace(TMP_Settings.fallbackFontPaths[i], m_FaceInfo.pointSize, TMP_Settings.fallbackFontFaceIndex[i]) != FontEngineError.Success)
                            {
                                continue;
                            }
                            ...
                            {
                                targetFontPath = TMP_Settings.fallbackFontPaths[i];
                                targetOSFallbackFontIndex = i;
                                break;
                            }
            ...
            targetFontFile = sourceFontFile;

            // 区分使用OS字时GlyphIndex，避免重复.Add by UITC.
            // 什么是重复的GlyphIndex？ Try this：✡光
            uint glyphRecordIndex = targetOSFallbackFontIndex == -1 ? glyphIndex : (uint)((targetOSFallbackFontIndex & 0x00FFFFFF) << 24) | glyphIndex;

            if(targetFontAsset == null)
            { 
                // Check if glyph is already contained in the font asset as the same glyph might be referenced by multiple characters.
                if (m_GlyphLookupDictionary.ContainsKey(glyphRecordIndex))
                {
                    character = new TMP_Character(unicode, this, m_GlyphLookupDictionary[glyphRecordIndex]);
                    m_CharacterTable.Add(character);
                    m_CharacterLookupDictionary.Add(unicode, character);
            ...
            {
                // 所有Fallback字打到主字体的AtlasTexture上
                return targetFontAsset.TryAddGlyphToTexture(unicode, glyphIndex, glyphRecordIndex, targetFontPath, targetFontFile, out character);
            }
            ...
            if (FontEngine.TryAddGlyphToTexture(glyphIndex, m_AtlasPadding, GlyphPackingMode.BestShortSideFit, m_FreeGlyphRects, m_UsedGlyphRects, m_AtlasRenderMode, m_AtlasTextures[m_AtlasTextureIndex], out glyph))
            {
                ...
                glyph.atlasIndex = m_AtlasTextureIndex;
                glyph.index = glyphRecordIndex;

                // Add new glyph to glyph table.
                m_GlyphTable.Add(glyph);
                m_GlyphLookupDictionary.Add(glyphRecordIndex, glyph);
                ...
                m_GlyphIndexList.Add(glyphRecordIndex);
                m_GlyphIndexListNewlyAdded.Add(glyphRecordIndex);
        ...
        internal bool TryAddGlyphToTexture(uint unicode, uint glyphIndex, uint glyphRecordIndex, string osFontPath, Font fontAsset, out TMP_Character character)
        {
            // Check if glyph is already contained in the font asset as the same glyph might be referenced by multiple characters.
            if (m_GlyphLookupDictionary.ContainsKey(glyphRecordIndex))
            {
            ...
            if (osFontPath != "")
            {
                FontEngine.LoadFontFace(osFontPath, m_FaceInfo.pointSize);
            }
            else if (fontAsset != null)
            { 
                FontEngine.LoadFontFace(fontAsset, m_FaceInfo.pointSize);
            }
            ...
            {
                OnFontAssetTextureAddFailed?.Invoke(this);    
            }
        ...
        internal void UpdateGlyphAdjustmentRecords()
        {
            ...
            GlyphPairAdjustmentRecord[] pairAdjustmentRecords = null;

            if (TMP_Settings.curLanguage == "cn" || TMP_Settings.curLanguage == "zh" || TMP_Settings.curLanguage == "tw" || TMP_Settings.curLanguage == "kr")
            {
                if (!string.IsNullOrEmpty(this.name) && !name.StartsWith("zh_cn_"))
                {
                ...
                pairAdjustmentRecords = FontEngine.GetGlyphPairAdjustmentTable(k_GlyphIndexArray);
            #else
                ...
                pairAdjustmentRecords = FontEngine.GetGlyphPairAdjustmentRecords(m_GlyphIndexList, out int recordCount);
            #endif
            ...
            {
                //OS 字跳过
                if (pairAdjustmentRecords[i].secondAdjustmentRecord.glyphIndex >> 24 != 0 || pairAdjustmentRecords[i].firstAdjustmentRecord.glyphIndex >> 24 != 0)
                    continue;
        ...
        public void ClearFontAssetData(bool setAtlasSizeToZero = false, bool sendEvent = false)
        {
...
#endif
                TMPro_EventManager.ON_FONT_ASSET_CLEAN();
            }
        }
        ...
        internal void ClearAtlasTextures(bool setAtlasSizeToZero = false)
        {
            ...
            texture = m_AtlasTexture = m_AtlasTextures[0];
                ...
                {
                    var asset = assets[i];
                    if (asset is Texture2D && asset != texture)
                    {
