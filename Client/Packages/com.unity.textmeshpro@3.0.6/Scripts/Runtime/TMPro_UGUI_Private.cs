...
namespace TMPro
{
    ...
    {
        [SerializeField] private bool m_hasFontAssetChanged = false; // Used to track when font properties have changed.
        ...
        [SerializeField] private Vector4 m_maskOffset;
        ...
        [NonSerialized] private bool m_isRegisteredForEvents;

        private bool m_isMeshInit;
        ...
        private static ProfilerMarker k_ThaiWordBreakMarker = new ProfilerMarker("TMP Thai Word Break");
...
#endif
                if (!TMP_TextInfo.TextInfoPoolSwitch)
                {
        ...
        protected override void OnEnable()
        {
            ...
            if (!m_isRegisteredForEvents)
            {
                ...
                TMPro_EventManager.FONT_ASSET_CLEAN.Add(onFontAssetClean);
            ...
            if (TMP_TextInfo.TextInfoPoolSwitch)
            {
                // Get new TextInfo for the text object.
                if (m_textInfo == null)
                {
                    m_textInfo = TMP_TextInfo.CreateTMP_TextInfo(this);
                }
                else
                {
                    TMP_TextInfo.ExchangeTextInfo(m_textInfo, m_InternalTextProcessingArraySize);
                }
            }
        }
        ...
        protected override void OnDisable()
        {
            ...
            if (TMP_TextInfo.TextInfoPoolSwitch)
            {
                TMP_TextInfo.ExchangeTextInfo(m_textInfo, 0, false, true);
                //TMP_TextInfo.Pool_Release(m_textInfo);
                //m_textInfo = TMP_TextInfo.defaultInfo;
            }
        }
        ...
        protected override void OnDestroy()
        {
            ...
            TMPro_EventManager.FONT_ASSET_CLEAN.Remove(onFontAssetClean);
        }

        protected virtual void onFontAssetClean()
        {
        ...
        internal override int SetArraySizes(UnicodeChar[] unicodeChars)
        {
            ...
            m_targetFontAsset = m_fontAsset.packAtlasTexture ? m_fontAsset : null;
            //If use sprite only, set sprite material as m_materialReferences[0]
            m_currentMaterial = m_useSpriteOnly ? m_spriteAsset.material : m_sharedMaterial;
            ...
            if(m_useSpriteOnly)
                MaterialReference.AddMaterialReference(m_currentMaterial, m_spriteAsset, ref m_materialReferences, m_materialReferenceIndexLookup);
            else
            ...
            //bool isDefaultInfo = false;// TMP_TextInfo.IsDefaultInfo(m_textInfo);
            if (m_textInfo == null)// || isDefaultInfo)
                m_textInfo = TMP_TextInfo.CreateTMP_TextInfo(m_InternalTextProcessingArraySize);
            else if (m_textInfo.characterInfo.Length < m_InternalTextProcessingArraySize)
            {
                TMP_TextInfo.ExchangeTextInfo(m_textInfo, m_InternalTextProcessingArraySize);//TMP_TextInfo.Resize(ref m_textInfo.characterInfo, m_InternalTextProcessingArraySize, false);//
            }
            ...
            */
            ...
            for (int i = 0; i < unicodeChars.Length && unicodeChars[i].unicode != 0; i++)
            {
                ...
                if (m_textInfo.characterInfo == null || m_totalCharacterCount >= m_textInfo.characterInfo.Length)
                    TMP_TextInfo.ExchangeTextInfo(m_textInfo, m_totalCharacterCount + 1, true);
                ...
                if (m_textElementType == TMP_TextElementType.Character)
                {
                    ...
                    {
                        if (TMP_Settings.curLanguage == "tr")
                        {
                            if (unicode == '\x0131') // ı -> I
                                unicode = '\x0049';
                            else if (unicode == '\x0069')
                                unicode = '\x0130'; // i -> İ
                        }
                    ...
                    else if ((m_FontStyleInternal & FontStyles.LowerCase) == FontStyles.LowerCase)
                    {
                        if (TMP_Settings.curLanguage == "tr")
                        {
                            if (unicode == '\x0049') // I -> ı
                                unicode = '\x0131';
                            else if (unicode == '\x0130') // İ -> i
                                unicode = '\x0069';
                        }
                ...
                if (character == null)
                {
                    ...
                    character = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)unicode, m_currentFontAsset, true, false, m_FontStyleInternal, m_FontWeightInternal, m_targetFontAsset, out isUsingAlternativeTypeface);

                    if (character == null)
                    {
                        // Search for the missing glyph character in the TMP Settings Fallback list.
                        if (TMP_Settings.fallbackFontAssets != null && TMP_Settings.fallbackFontAssets.Count > 0)
                            character = TMP_FontAssetUtilities.GetCharacterFromFontAssets((uint)unicode, m_currentFontAsset, TMP_Settings.fallbackFontAssets, true, false, m_FontStyleInternal, m_FontWeightInternal, m_targetFontAsset, out isUsingAlternativeTypeface);
                    }
                    ...
                    if (character == null)
                    {
                        ...
                        if (TMP_Settings.defaultFontAsset != null)
                            character = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)unicode, TMP_Settings.defaultFontAsset, true, false, m_FontStyleInternal, m_FontWeightInternal, m_targetFontAsset, out isUsingAlternativeTypeface);
                    }
                    ...
                    if (character == null)
                    {
                        // Search for the missing glyph in the TMP Settings Default OS Font Asset.
                        if ( /*m_useOSFontFallback &&*/ TMP_Settings.defaultOSFontAsset != null)
                        {
                            character = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)unicode, TMP_Settings.defaultOSFontAsset, true, true, m_FontStyleInternal, m_FontWeightInternal, m_targetFontAsset, out isUsingAlternativeTypeface);
                        }
                    }
                ...
                if (character.elementType == TextElementType.Sprite)
                {
                    ...
                    if (m_spriteMaterialIndex == 0)
                        ...
                        m_currentMaterialIndex = MaterialReference.AddMaterialReference(spriteAssetRef.AlternativeMaterials[m_spriteMaterialIndex - 1], spriteAssetRef, ref m_materialReferences, m_materialReferenceIndexLookup);
                    m_materialReferences[m_currentMaterialIndex].referenceCount += 1;
            ...
            if (m_VertexBufferAutoSizeReduction && m_textInfo.characterInfo.Length - m_totalCharacterCount > 256)
                TMP_TextInfo.ExchangeTextInfo(m_textInfo, Mathf.Max(m_totalCharacterCount + 1, 256), true);
            ...
            for (int i = 0; i < materialCount; i++)
            {
                ...
                if (i > 0)
                {
                    ...
                    {
                        if (m_subTextObjects[i].sharedMaterial != null && !m_subTextObjects[i].sharedMaterial.name.Contains("_ForceStencilSet"))
        ...
        internal override void InternalUpdate()
        {
            ...
            if( needLocalizedUpdate )
            {
                ForceMeshUpdate();
                needLocalizedUpdate = false;
            }
        }
        ...
        void OnPreRenderCanvas()
        {
            ...
            if (m_havePropertiesChanged || m_isLayoutDirty)
            {
                ...
                if( m_enableAutoSizing && !(m_autoSizeInitOnce && m_IsAutoSizeInited))
                ...
                while (m_IsAutoSizePointSizeSet == false)
                {
                    ...
                    //if (m_AutoSizeIterationCount > 1)
                    //{
                    //    Debug.LogFormat("[TATest] generate text cause auto size multi time {0} {1}", m_text, gameObject.GetInstanceID());
                    //}
                }
            }
        }
        ...
        {
            //string trackStr = new System.Diagnostics.StackTrace(true).ToString();
            //Debug.LogFormat("[TATest] call generate text {0} {1}", m_text, trackStr);
            ...
            m_ItalicAngle = TMP_Settings.curLanguage == "kr" ? 0 : (m_uesItalicOverride ? m_italicOverrideValue : m_currentFontAsset.italicStyle);
            ...
            m_ThaiWordEndIndex = -1;
            bool thaiBasicCharMark = false;
            int lastSoftLineBreak = 0;
            // when language == eng, and first char is punctuation
            bool isEnglishPunctuation = false;
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                {
                    if ((m_overflowMode == TextOverflowModes.Linked && m_linkedTextComponent != null) || (m_disableCharacterSpacingInThai == true))
                        characterSpacingAdjustment = 0;
                    else if (i < m_TextProcessingArray.Length - 1)
                    {
                        var nextCharCode = m_TextProcessingArray[i + 1].unicode;
                        if (!ThaiFontAdjuster.IsBaseOrNotThai((char)nextCharCode)) //Is Deco
                        {
                            characterSpacingAdjustment = 0;
                        }
                    }
                ...
                {
                    // Add By UITC : 英文首位为装饰符时，缩减characterSpacing以挤入后续字符
                    var firstCharCode = m_TextProcessingArray[0].unicode;
                    bool isPunctuation = EngFontAdjuster.IsDecoLetter((char)firstCharCode);
                    if (isPunctuation && m_overflowMode == TextOverflowModes.Linked && m_linkedTextComponent != null && m_LinkedOverflowExtraMode == LinkedOverflowExtraModes.SingleChar)
                        characterSpacingAdjustment = 0;
                }
                ...
                if (charCode == 9 || (isWhiteSpace == false && charCode != 0x200B && charCode != 0xAD && charCode != 0x03) || (charCode == 0xAD && isSoftHyphenIgnored == false) || m_textElementType == TMP_TextElementType.Sprite)
                {
                    ...
                    if (textWidth > widthOfTextArea * (isJustifiedOrFlush ? 1.05f : 1.0f))
                    {
                        ...
                        if (m_enableWordWrapping && m_characterCount != m_firstCharacterOfLine)
                        {
                            ...
                            if( m_enableAutoSizing && !(m_autoSizeInitOnce && m_IsAutoSizeInited) && isFirstWordOfLine )
                            {
                        ...
                        {
                            if( m_enableAutoSizing && !(m_autoSizeInitOnce && m_IsAutoSizeInited) && m_AutoSizeIterationCount < m_AutoSizeMaxIterationCount )
                            {
                                // Handle Character Width Adjustments
                    ...
                    #region LinkedOverflow Extra Mode Check
                    if (m_overflowMode == TextOverflowModes.Linked)
                    {
                        if (m_LinkedOverflowExtraMode == LinkedOverflowExtraModes.SingleChar)
                        {
                            if (TMP_Settings.curLanguage == "th")
                            {
                                if (m_characterCount == 0)
                                {
                                    thaiBasicCharMark = ThaiFontAdjuster.IsBaseOrNotThai((char)charCode);
                                }
                                else if (m_characterCount > 0)
                                {
                                    bool isBasic = ThaiFontAdjuster.IsBaseOrNotThai((char)charCode);
                                    if ((!thaiBasicCharMark && m_characterCount > 0) || thaiBasicCharMark && isBasic)
                                    {
                                        // Set isTextOverflowing and firstOverflowCharacterIndex
                                        if (m_firstOverflowCharacterIndex == -1)
                                            m_firstOverflowCharacterIndex = m_characterCount;
                            ...
                            else
                            {
                                if (m_characterCount == 0)
                                {
                                    isEnglishPunctuation = EngFontAdjuster.IsDecoLetter((char)charCode);
                                }
                                else if (m_characterCount > 0)
                                {
                                    bool isPunctuation = EngFontAdjuster.IsDecoLetter((char)charCode);
                                    //当前字符是非装饰符，且之前已经出现过非装饰符
                                    if (!isPunctuation && !isEnglishPunctuation && m_linkedTextComponent != null)
                                    {
                                        // Set isTextOverflowing and firstOverflowCharacterIndex
                                        if (m_firstOverflowCharacterIndex == -1)
                                            m_firstOverflowCharacterIndex = m_characterCount;
                                    ...
                                    isEnglishPunctuation = isEnglishPunctuation && isPunctuation;
                                }
                            }
                        }
                    }
                ...
                if (m_enableWordWrapping || m_overflowMode == TextOverflowModes.Truncate || m_overflowMode == TextOverflowModes.Ellipsis || m_overflowMode == TextOverflowModes.Linked)
                {
                    ...
                    else if(ThaiFontAdjuster.enable && ThaiFontAdjuster.enableThaiWordTree &&
                            (charCode > 0x0E00 && charCode < 0x0E7F ||
                             charCode > 0xF6FF && charCode < 0xF72C) ) /* Thai */
                    {
                        k_ThaiWordBreakMarker.Begin();

                        // Check if the character is within punctuation Unicode ranges
                        bool isPunctuation =
                            (charCode >= 0x0021 && charCode <= 0x002F) ||  // Basic Latin punctuation
                            ...
                            (charCode >= 0xFF01 && charCode <= 0xFF0F) ||  // Fullwidth punctuation
                        ...
                        if ( isFirstWordOfLine )
                        {
                            ...
                            if(m_ThaiWordEndIndex == -1)
                            {
                                m_ThaiWordEndIndex = GetNextThaiBreakPos(m_TextProcessingArray[i].stringIndex);
                            }

                            if (isPunctuation)
                            {
                                // If punctuation is at the beginning, wrap it with the previous word
                                if (m_characterCount > 0)
                                {
                                    SaveWordWrappingState(ref m_SavedWordWrapState, i - 1, m_characterCount - 1);
                                    isFirstWordOfLine = false;
                                }
                            }

                            else if (m_ThaiWordEndIndex == -1)
                            {
                                SaveWordWrappingState( ref m_SavedWordWrapState, i, m_characterCount );
                            }
                            else 
                            {
                                if( m_TextProcessingArray[i].stringIndex >= m_ThaiWordEndIndex )
                                {
                                    //char curchar = m_text[ i ];
                                    isFirstWordOfLine = false;
                                    SaveWordWrappingState( ref m_SavedWordWrapState, i, m_characterCount );
                                    m_ThaiWordEndIndex = -1;
                                }
                        ...
                        else
                        {
                            ...
                            else if( m_TextProcessingArray[i].stringIndex >= m_ThaiWordEndIndex )
                            {
                                SaveWordWrappingState( ref m_SavedWordWrapState, i, m_characterCount );
                                m_ThaiWordEndIndex = -1;
                            }
                        }
                        k_ThaiWordBreakMarker.End();
                    }
            ...
            if(/* !m_isCharacterWrappingEnabled && */ m_enableAutoSizing && !(m_autoSizeInitOnce && m_IsAutoSizeInited) && fontSizeDelta > 0.051f && m_fontSize < m_fontSizeMax && m_AutoSizeIterationCount < m_AutoSizeMaxIterationCount )
            {
                // Reset character width adjustment delta
                if( m_charWidthAdjDelta < m_charWidthMaxAdj / 100 )
                    m_charWidthAdjDelta = 0;
            ...
            int totalLine = 0;
            for (int i = m_characterCount - 1; i >= 0; i--)
            {
                if (characterInfos[i].isVisible)
                {
                    totalLine = characterInfos[i].lineNumber + 1;
                    break;
                }
            }
            ...
            for (int i = 0; i < m_characterCount; i++)
            {
                ...
                if (isCharacterVisible)
                {
                    ...
                    float minY = characterInfos[i].vertex_BL.uv.y;
                    float maxY = characterInfos[i].vertex_TL.uv.y;
                    characterInfos[i].vertex_BL.uv4 = new Vector2(PackUV(minY, maxY), PackIntToFloat(currentLine, totalLine));
                    characterInfos[i].vertex_TL.uv4 = new Vector2(PackUV(minY, maxY), PackIntToFloat(currentLine, totalLine));
                    characterInfos[i].vertex_TR.uv4 = new Vector2(PackUV(minY, maxY), PackIntToFloat(currentLine, totalLine));
                    characterInfos[i].vertex_BR.uv4 = new Vector2(PackUV(minY, maxY), PackIntToFloat(currentLine, totalLine));
                    //float minY = characterInfos[i].vertex_BL.position.y;
                    //float maxY = characterInfos[i].vertex_TL.position.y;
                    //characterInfos[i].vertex_BL.uv4 = new Vector2(PackUV(minY, maxY), PackIntToFloat(currentLine, totalLine));
                    //characterInfos[i].vertex_TL.uv4 = new Vector2(PackUV(minY, maxY), PackIntToFloat(currentLine, totalLine));
                    //characterInfos[i].vertex_TR.uv4 = new Vector2(PackUV(minY, maxY), PackIntToFloat(currentLine, totalLine));
                    //characterInfos[i].vertex_BR.uv4 = new Vector2(PackUV(minY, maxY), PackIntToFloat(currentLine, totalLine));
                    ...
                    {
                        FillCharacterVertexBuffers(i, vert_index_X4, currentLine);
                    }
                    else if (elementType == TMP_TextElementType.Sprite)
                    {
                        FillSpriteVertexBuffers(i, sprite_index_X4, currentLine);
                    }
                }
            ...
            if (m_renderMode == TextRenderFlags.Render && IsActive())
            {
                ...
                //UITA Test
                if (m_textInfo.meshInfo[0].uvs2.Length != m_textInfo.meshInfo[0].vertices.Length)
                {
                    Debug.LogWarning("TATest : TMP UV2 Array ERROR");
                    var temp = new Vector2[m_textInfo.meshInfo[0].vertices.Length];
                    for (int i = 0; i < m_textInfo.meshInfo[0].vertices.Length; i++)
                    {
                        temp[i] = m_textInfo.meshInfo[0].uvs2[i];
                    }

                    m_textInfo.meshInfo[0].uvs2 = temp;
                }
        ...
        void UpdateSDFScale(float scaleDelta)
        {
            ...
            for (int i = 0; i < m_textInfo.materialCount; i++)
            {
                if (i == 0)
                {
                    if (m_mesh.uv2 != null && m_mesh.uv2.Length > m_textInfo.meshInfo[0].uvs2.Length)
                    {
                        for (int j = 0; j < m_textInfo.meshInfo[0].uvs2.Length; j++)
                            m_mesh.uv2[j] = m_textInfo.meshInfo[0].uvs2[j];
                    }
                    else
                        m_mesh.uv2 = m_textInfo.meshInfo[0].uvs2;
                ...
                {
                    //UIShadow resize vertices bug temp resolution 
                    if (m_subTextObjects[i].mesh.uv2 != null && m_subTextObjects[i].mesh.uv2.Length > m_textInfo.meshInfo[i].uvs2.Length)
                    {
                        for (int j = 0; j < m_textInfo.meshInfo[i].uvs2.Length; j++)
                            m_subTextObjects[i].mesh.uv2[j] = m_textInfo.meshInfo[i].uvs2[j];
                    }
                    else
                        m_subTextObjects[i].mesh.uv2 = m_textInfo.meshInfo[i].uvs2;
        ...
        //    if( ThaiFontAdjuster.enable && ThaiFontAdjuster.IsThaiString( text ) )
        //    {
        //        var newText = ThaiFontAdjuster.Adjust( text );
        //        if( !newText.Equals( text ) )
        //        {
        //            text = newText;
        //            needLocalizedUpdate = true;
        //        }
        //    }
        //}
