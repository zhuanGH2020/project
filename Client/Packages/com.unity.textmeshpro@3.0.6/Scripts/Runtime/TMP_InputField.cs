...
namespace TMPro
{
        ...
        IScrollHandler
    {
        ...
        public bool CaretVisible => m_CaretVisible;
        ...
        protected int caretPositionInternal { get { return m_CaretPosition + compositionLength; } set { m_CaretPosition = value; ClampCaretPos(ref m_CaretPosition); MatchEmojiPartAndDelete(); } }
                ...
                else
                {
                    ...
                    for (int i = 0; i < val.Length; ++i)
                    {
                        ...
                        if (_checkWhitelist != null)
                        {
                            ...
                            ignoreValidate = _checkWhitelist(c, c1);
                        }
        ...
        public void Insert(string s)
        {
            ...
            var value = text.Insert(m_StringPosition, s);
            value = value.Replace("\0", string.Empty);
            m_Text = value;
            if (m_SoftKeyboard != null)
                m_SoftKeyboard.text = m_Text;
            m_CaretSelectPosition = m_CaretPosition += s.Length;
            m_StringSelectPosition = m_StringPosition += s.Length;
            m_forceRectTransformAdjustment = true;
            m_IsTextComponentUpdateRequired = true;
            UpdateLabel();
            SendOnValueChanged();
        ...
        public virtual void Append(string input)
        {
            ...
            for (int i = 0, imax = input.Length; i < imax; ++i)
            {
                ...
                if (_checkWhitelist != null)
                {
                    ...
                    var isWhitelist = _checkWhitelist(c, c1);
        ...
        protected virtual void Append(char input, bool ignoreValidate = false)
        {
            if (m_ReadOnly)
                return;
        ...
        {
#if UNITY_EDITOR
             OnTMPInputFieldEndEditRecordHandler?.Invoke(gameObject,m_Text);
#endif
            if (onEndEdit != null)
        ...
        const string patternStr = @"(<sprite=\d\d(?!>)|<sprite=\d(?!(\d|>)))";
        const string replaceStr = "";
        public void MatchEmojiPartAndDelete()
        {
            text = Regex.Replace(text, patternStr, replaceStr);
        }

        public delegate bool DelegateCheckWhitelist(char c, char c1);
        public DelegateCheckWhitelist _checkWhitelist;
     
        public void SetWhitelistChecker(DelegateCheckWhitelist checkWhitelist)
        {
