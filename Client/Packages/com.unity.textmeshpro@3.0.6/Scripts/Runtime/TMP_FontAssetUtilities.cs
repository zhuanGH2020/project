...
namespace TMPro
{
    public class TMP_FontAssetUtilities
    {
        ...
        /// <returns></returns>
        public static TMP_Character GetCharacterFromFontAsset(uint unicode, TMP_FontAsset sourceFontAsset, bool includeFallbacks, bool includeOSFallbacks, FontStyles fontStyle, FontWeight fontWeight, TMP_FontAsset targetFontAsset, out bool isAlternativeTypeface)
        {
            if (includeFallbacks)
            {
            ...
            return GetCharacterFromFontAsset_Internal(unicode, sourceFontAsset, includeFallbacks, includeOSFallbacks, fontStyle, fontWeight, targetFontAsset, out isAlternativeTypeface);
        }
        ...
        /// </summary>
        private static TMP_Character GetCharacterFromFontAsset_Internal(uint unicode, TMP_FontAsset sourceFontAsset, bool includeFallbacks, bool includeOSFallbacks, FontStyles fontStyle, FontWeight fontWeight, TMP_FontAsset targetFontAsset, out bool isAlternativeTypeface)
        {
            isAlternativeTypeface = false;
            TMP_Character character = null;

            //if (!includeOSFallbacks && sourceFontAsset.useOSFontFallback)
            //    return null;
            ...
            if (isItalic || fontWeight != FontWeight.Regular)
            {
                ...
                if ((targetFontAsset == sourceFontAsset || targetFontAsset == null) && temp != null)
                {
                    ...
                    {
                        if (temp.TryAddCharacterInternal(unicode, includeOSFallbacks, null, out character))
                        {
                            isAlternativeTypeface = true;
            ...
            // TODO: 不直接返回OsFallback逻辑
            // Search the source font asset for the requested character.
            if((targetFontAsset == sourceFontAsset || targetFontAsset == null) && sourceFontAsset.characterLookupTable.TryGetValue(unicode, out character))
                return character;

            if (sourceFontAsset.atlasPopulationMode == AtlasPopulationMode.Dynamic /*&& !(!includeOSFallbacks && sourceFontAsset.useOSFontFallback)*/)
            {
                if (sourceFontAsset.TryAddCharacterInternal(unicode, includeOSFallbacks, targetFontAsset, out character))
                    return character;
            }
            ...
            if (character == null && includeFallbacks && sourceFontAsset.fallbackFontAssetTable != null)
            {
                ...
                for (int i = 0; i < fallbackCount; i++)
                {
                    ...
                    //if (!includeOSFallbacks && temp.useOSFontFallback)
                    //    continue;
                    ...
                    character = GetCharacterFromFontAsset_Internal(unicode, temp, true, includeOSFallbacks, fontStyle, fontWeight, targetFontAsset, out isAlternativeTypeface);
        ...
        /// <returns></returns>
        public static TMP_Character GetCharacterFromFontAssets(uint unicode, TMP_FontAsset sourceFontAsset, List<TMP_FontAsset> fontAssets, bool includeFallbacks, bool includeOSFallbacks, FontStyles fontStyle, FontWeight fontWeight, TMP_FontAsset targetFontAsset, out bool isAlternativeTypeface)
        {
            isAlternativeTypeface = false;
            ...
            for (int i = 0; i < fontAssetCount; i++)
            {
                ...
                TMP_Character character = GetCharacterFromFontAsset_Internal(unicode, fontAsset, includeFallbacks, includeOSFallbacks, fontStyle, fontWeight, targetFontAsset, out isAlternativeTypeface);
