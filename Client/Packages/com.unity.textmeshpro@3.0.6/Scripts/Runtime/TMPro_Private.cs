        ...
        internal override int SetArraySizes(UnicodeChar[] unicodeChars)
        {
            ...
            */
            ...
            for (int i = 0; i < unicodeChars.Length && unicodeChars[i].unicode != 0; i++)
            {
                ...
                if (character == null)
                {
                    // UITC: Private上没有开PackToAtlasTexture功能
                    ...
                    character = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)unicode, m_currentFontAsset, true, false, m_FontStyleInternal, m_FontWeightInternal, null, out isUsingAlternativeTypeface);

                    if (character == null)
                    {
                        // Search for the missing glyph character in the TMP Settings Fallback list.
                        if (TMP_Settings.fallbackFontAssets != null && TMP_Settings.fallbackFontAssets.Count > 0)
                            character = TMP_FontAssetUtilities.GetCharacterFromFontAssets((uint)unicode, m_currentFontAsset, TMP_Settings.fallbackFontAssets, true, false, m_FontStyleInternal, m_FontWeightInternal, null, out isUsingAlternativeTypeface);
                    }
                    ...
                    if (character == null)
                    {
                        ...
                        if (TMP_Settings.defaultFontAsset != null)
                            character = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)unicode, TMP_Settings.defaultFontAsset, true, false, m_FontStyleInternal, m_FontWeightInternal, null, out isUsingAlternativeTypeface);
                    }
                    ...
                    if (character == null)
                    {
                        ...
                        {
                            character = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)unicode, TMP_Settings.defaultOSFontAsset, false, true, m_FontStyleInternal, m_FontWeightInternal, null, out isUsingAlternativeTypeface);
                        }
                    }
            ...
            for (int i = 0; i < materialCount; i++)
            {
                ...
                if (m_textInfo.meshInfo[i].vertices == null || m_textInfo.meshInfo[i].vertices.Length < referenceCount * 4)
                {
                    ...
                    {
                        if (i == 0)
                            m_textInfo.meshInfo[i] = new TMP_MeshInfo(m_mesh, referenceCount + 1, false, m_lineNumber);
                        else
                            m_textInfo.meshInfo[i] = new TMP_MeshInfo(m_subTextObjects[i].mesh, referenceCount + 1, false, m_lineNumber);
                    }
                    else
        ...
        protected virtual void GenerateTextMesh()
        {
            ...
            for (int i = 0; i < m_characterCount; i++)
            {
                ...
                if (isCharacterVisible)
                {
                    ...
                    {
                        FillCharacterVertexBuffers(i, vert_index_X4, false, currentLine);
                    }
            ...
            if (m_renderMode == TextRenderFlags.Render && IsActive())
            {
                ...
                for (int i = 1; i < m_textInfo.materialCount; i++)
                {
                    ...
                    if (m_textInfo.meshInfo[0].uvs2.Length != m_textInfo.meshInfo[0].vertices.Length)
                    {
                        ...
                        for (int k = 0; k < m_textInfo.meshInfo[0].vertices.Length; k++)
                        {
                            temp[k] = m_textInfo.meshInfo[0].uvs2[k];
                        }
