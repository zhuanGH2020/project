...
namespace UnityEditor.UI
{
    ...
    public class ImageEditor : GraphicEditor
    {
        ...
        AnimBool m_ShowType;
        bool m_bIsDriven;
        SerializedProperty m_CustomUV;
        SerializedProperty m_UVRect;
        GUIContent m_UVRectContent;
        SerializedProperty m_Round;
        SerializedProperty m_RoundCorners;
        SerializedProperty m_RoundRadius;
        SerializedProperty m_RoundTriangles;
        ...
        protected override void OnEnable()
        {
            ...
            m_CustomUV        = serializedObject.FindProperty("m_CustomUV");
            ...
            m_RoundRadius = serializedObject.FindProperty("m_RoundRadius");
            m_RoundTriangles = serializedObject.FindProperty("m_RoundTriangles");
        ...
        public override void OnInspectorGUI()
        {
            ...
            if ((Image.Type) m_Type.enumValueIndex == Image.Type.Simple && !m_UseSpriteMesh.boolValue)
            {
                EditorGUILayout.PropertyField(m_CustomUV);
            }
            else
            {
                m_CustomUV.boolValue = false;
            }

            if (m_CustomUV.boolValue)
            {
                EditorGUI.indentLevel++;
            ...
            if ((Image.Type) m_Type.enumValueIndex == Image.Type.Simple && !m_UseSpriteMesh.boolValue)
            {
                EditorGUILayout.PropertyField(m_Round);
            }
            else
            {
                m_Round.boolValue = false;
            }

            if (m_Round.boolValue)
            {
                EditorGUI.indentLevel++;
                EditorGUILayout.PropertyField(m_RoundCorners);
                EditorGUILayout.PropertyField(m_RoundRadius);
                EditorGUILayout.PropertyField(m_RoundTriangles);
                if (m_RoundTriangles.intValue == 0)
                {
                    m_RoundTriangles.intValue = 1;
                }
                EditorGUI.indentLevel--;
            }
