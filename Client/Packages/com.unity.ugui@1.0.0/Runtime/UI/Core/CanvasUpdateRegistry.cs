    ...
    public class CanvasUpdateRegistry
    {
...
#if UNITY_EDITOR
        public static event Action<IEnumerable<ICanvasElement>> OnCanvasElementsRebuild;
#endif
        ...
        private void PerformUpdate()
        {
            ...
            for (var i = (int)CanvasUpdate.PreRender; i < (int)CanvasUpdate.MaxUpdateValue; i++)
            {
                ...
                // UITC copy
                // 触发事件并传递 List 的副本
                IEnumerable<ICanvasElement> graphicQueueList = GetInternalList(m_GraphicRebuildQueue);
                OnCanvasElementsRebuild?.Invoke(graphicQueueList);
#endif
...
#if UNITY_EDITOR
        private List<ICanvasElement> GetInternalList(IndexedSet<ICanvasElement> indexedSet)
        {
            FieldInfo listField = typeof(IndexedSet<ICanvasElement>).GetField("m_List", BindingFlags.NonPublic | BindingFlags.Instance);
            return listField?.GetValue(indexedSet) as List<ICanvasElement> ?? new List<ICanvasElement>();
        }
#endif
