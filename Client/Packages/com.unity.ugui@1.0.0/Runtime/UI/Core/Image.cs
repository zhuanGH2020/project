...
namespace UnityEngine.UI
{
    ...
    public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        ...
        /// Image's dimensions used for drawing. X = left, Y = bottom, Z = right, W = top.
        private Vector4 GetRoundedDrawingDimensions(bool shouldPreserveAspect)
        {
            var padding = activeSprite == null ? Vector4.zero : new Vector4(0, 0, 1, 1);
            var r = GetPixelAdjustedRect();
            var size = activeSprite == null ? new Vector2(r.width, r.height) : new Vector2(activeSprite.rect.width, overrideSprite.rect.height);
            //Debug.Log(string.Format("r:{2}, size:{0}, padding:{1}", size, padding, r));
            ...
            switch (type)
            {
                case Type.Simple:
                    if (!useSpriteMesh)
                    {
                        ...
                        {
                            GenerateRoundedSprite(toFill, m_PreserveAspect);                            
                        }
                    }
                    else
        ...
        void GenerateSimpleSprite(VertexHelper vh, bool lPreserveAspect)
        {
            ...
            if (m_CustomUV)
            {
                uv.x = m_UVRect.xMin * (uv.z - uv.x) + uv.x;
                uv.y = m_UVRect.yMin * (uv.w - uv.y) + uv.y;
                uv.z = (m_UVRect.xMax - 1) * (uv.z - uv.x) + uv.z;
                uv.w = (m_UVRect.yMax - 1) * (uv.w - uv.y) + uv.w;
            }

            var color32 = color;
            vh.Clear();

            vh.AddVert(new Vector3(v.x, v.y), color32, new Vector2(uv.x, uv.y), new Vector2 (0,0));
            vh.AddVert(new Vector3(v.x, v.w), color32, new Vector2(uv.x, uv.w), new Vector2 (0,1));
            vh.AddVert(new Vector3(v.z, v.w), color32, new Vector2(uv.z, uv.w), new Vector2 (1,1));
            vh.AddVert(new Vector3(v.z, v.y), color32, new Vector2(uv.z, uv.y), new Vector2 (1,0));

            vh.AddTriangle(0, 1, 2);
            vh.AddTriangle(2, 3, 0);
        }

        private void GenerateRoundedSprite(VertexHelper vh, bool lPreserveAspect)
        {
            var v = GetDrawingDimensions(lPreserveAspect);
            var uv = activeSprite != null ? Sprites.DataUtility.GetOuterUV(activeSprite) : Vector4.zero;
            ...
            var radius = m_RoundRadius;
            radius = Mathf.Clamp(radius, 0, (v.z - v.x) / 2);
            radius = Mathf.Clamp(radius, 0, (v.w - v.y) / 2);
            
            var uvRadiusX = radius / (v.z - v.x) * (uv.z - uv.x);
            var uvRadiusY = radius / (v.w - v.y) * (uv.w - uv.y);
            var isLeftTop = (m_RoundCorners & Corners.LeftTop) != 0;
            var isLeftBottom = (m_RoundCorners & Corners.LeftBottom) != 0;
            var isRightTop = (m_RoundCorners & Corners.RightTop) != 0;
            var isRightBottom = (m_RoundCorners & Corners.RightBottom) != 0;
            ...
            //每个三角形的顶角
            var degreeDelta = Mathf.PI / 2 / m_RoundTriangles;
            //当前的角度
            var curDegree = 0f;

            for (var i = 0; i < vCenterVertList.Count; i++)
            {
                var vCenter = vCenterVertList[i];
                curDegree = vCenter switch
                {
                    7 => 0,
                    3 => Mathf.PI / 2,
                    4 => Mathf.PI,
                ...
                for (var j = 0; j <= m_RoundTriangles; j++)
                {
                ...
                curDegree -= degreeDelta;
                for (var j = 0; j <= m_RoundTriangles - 1; j++)
                {
                    vh.AddTriangle(vCenter, preVertNum + j + 1, preVertNum + j);
                }
            }
        }
