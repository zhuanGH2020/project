...
/// </summary>
public class PlantBullet : ObjectBase
{
    [Header("子弹设置")]
                          ...
                          float speed = 15f, float maxDistance = 10f, int bulletConfigId = 0)
    {
        ...
        Debug.Log($"[PlantBullet] 初始化子弹 - 伤害:{_damage}, 速度:{_speed}, 最大距离:{_maxDistance}");
    }
    ...
    private void LoadBulletConfigValues(int bulletConfigId)
    {
        ...
        {
            Debug.LogWarning($"[PlantBullet] 子弹配置ID {bulletConfigId} 不存在，使用默认参数");
            return;
        }
        ...
        // 例如：爆炸范围、特效路径、音效等
        // 这里暂时保留接口，后续可扩展
        Debug.Log($"[PlantBullet] 从配置表加载子弹参数: {bulletConfigId}");
    }
    ...
    private void CheckCollision()
    {
        ...
        foreach (var hitCollider in hitColliders)
        {
            ...
            // 检查是否是其他可承伤对象（但不是植物自己）
            var damageable = hitCollider.GetComponent<IDamageable>();
            if (damageable != null && !IsPlant(hitCollider))
            {
    ...
    /// 检查是否是植物对象（避免误伤）
    /// </summary>
    private bool IsPlant(Collider collider)
    {
    ...
    private void DealDamage(IDamageable target, Vector3 hitPoint)
    {
        ...
        Debug.Log($"[PlantBullet] 击中目标，造成 {actualDamage} 伤害");
    ...
    private void CheckMaxDistance()
    {
        ...
        {
            Debug.Log($"[PlantBullet] 达到最大距离 {_maxDistance}，子弹消失");
            DestroyBullet();
        }
    }
